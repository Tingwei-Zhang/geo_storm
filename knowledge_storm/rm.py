...
class SerperRM(dspy.Retrieve):
    ...
    def __init__(
        ...
        enable_arctic_shift: Use Arctic Shift API for Reddit content (default False).
        merge_snippets: Merge SERP snippet + extracted page snippets into one (default False).
        """
        super().__init__(k=k)
        self.usage = 0
        ...
        for result in self.results:
            try:
                ...
                for organic in organic_results:
                    ...
                    if self.ENABLE_EXTRA_SNIPPET_EXTRACTION:
                        extra = valid_url_to_snippets.get(url, {}).get("snippets", [])
                        if extra:
                            if self.merge_snippets:
                                merged = "\n\n".join(
